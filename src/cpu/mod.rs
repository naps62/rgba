...
impl CPU {
  ...
  pub fn exec(&mut self) {
    ...
    let byte = self.mmu.read8(current_pc as usize);
  ...
  fn exec_opcode(&mut self, opcode: u8, pc: u16) -> u16 {
    ...
    match opcode {
      ...
      // LD (N), SP
      0x08 => {
        self.mmu.write16(self.read_arg16() as usize, self.regs.sp());
        pc + 3
      }
      ...
      // LD (R), A
      0b0000_0010 => {
        self.mmu.write8(self.regs.bc() as usize, self.regs.a());
        pc + 1
      }
      0b0001_0010 => {
        self.mmu.write8(self.regs.de() as usize, self.regs.a());
        pc + 1
      }

      // LD A, (R)
      0b0000_1010 => {
        self.regs.write8(A, self.mmu.read8(self.regs.bc() as usize));
        pc + 1
      }
      0b0001_1010 => {
        self.regs.write8(A, self.mmu.read8(self.regs.de() as usize));
        pc + 1
      }
      ...
      0b0011_0100 => {
        ...
        let v = self.alu_inc(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
        pc + 1
      }
      0b0011_1100 => {
      ...
      0b0011_0101 => {
        let ptr: usize = self.regs.hl() as usize;
        let v = self.alu_dec(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
        pc + 1
      }
      0b0011_1101 => {
      ...
      0b0011_0110 => {
        let ptr: usize = self.regs.hl() as usize;
        self.mmu.write8(ptr, self.read_arg8());
        pc + 2
      }
      0b0011_1110 => {
      ...
      // LDI (HL), A
      0b0010_0010 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.a());
        self.regs.set_hl(self.regs.hl().wrapping_add(1));
        pc + 1
      }
      ...
      // LDI A, (HL)
      0b0010_1010 => {
        self.regs.set_a(self.mmu.read8(self.regs.hl() as usize));
        self.regs.set_hl(self.regs.hl().wrapping_add(1));
        pc + 1
      }
      ...
      0b0100_0110 => {
        self.regs.set_b(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0100_0111 => {
      ...
      0b0100_1110 => {
        self.regs.set_c(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0100_1111 => {
      ...
      0b0101_0110 => {
        self.regs.set_d(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0101_0111 => {
      ...
      0b0101_1110 => {
        self.regs.set_e(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0101_1111 => {
      ...
      0b0110_0110 => {
        self.regs.set_h(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0110_0111 => {
      ...
      0b0110_1110 => {
        self.regs.set_l(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0110_1111 => {
      ...
      // LD (HL), r8
      0b0111_0000 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.b());
        pc + 1
      }
      0b0111_0001 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.c());
        pc + 1
      }
      0b0111_0010 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.d());
        pc + 1
      }
      0b0111_0011 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.e());
        pc + 1
      }
      0b0111_0100 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.h());
        pc + 1
      }
      0b0111_0101 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.l());
        pc + 1
      }
      0b0111_0111 => {
        self.mmu.write8(self.regs.hl() as usize, self.regs.a());
      ...
      0b0111_1110 => {
        self.regs.set_a(self.mmu.read8(self.regs.hl() as usize)); // no-op
        pc + 1
      }
      0b0111_1111 => {
        self.regs.set_a(self.regs.a());
      ...
      0b1000_0110 => alu_add_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1000_0111 => alu_add_a!(self.regs.a()),
      // ADD A, N
      0b1100_0110 => {
        alu_add_a!(self.read_arg8());
        2
      }
      ...
      0b1000_1110 => alu_adc_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1000_1111 => alu_adc_a!(self.regs.a()),
      // ADC A, N
      0b1100_1110 => {
        alu_adc_a!(self.read_arg8());
        2
      }
      ...
      0b1001_0110 => alu_sub_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1001_0111 => alu_sub_a!(self.regs.a()),
      // SUB A, N
      0b1101_0110 => {
        alu_sub_a!(self.read_arg8());
        2
      }
      ...
      0b1001_1110 => alu_sbc_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1001_1111 => alu_sbc_a!(self.regs.a()),
      // sbc A, N
      0b1101_1110 => {
        alu_sbc_a!(self.read_arg8());
        2
      }
      ...
      0b1010_0110 => alu_and_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1010_0111 => alu_and_a!(self.regs.a()),
      // ADD A, N
      0b1110_0110 => {
        alu_and_a!(self.read_arg8());
        2
      }
      ...
      0b1010_1110 => alu_xor_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1010_1111 => alu_xor_a!(self.regs.a()),
      // ADD A, N
      0b1110_1110 => {
        alu_xor_a!(self.read_arg8());
        2
      }
      ...
      0b1011_0110 => alu_or_a!(self.mmu.read8(self.regs.hl() as usize)),
      0b1011_0111 => alu_or_a!(self.regs.a()),
      // OR A, N
      0b1111_0110 => {
        alu_or_a!(self.read_arg8());
        2
      }
      ...
      0b1110_0000 => {
        ...
        self.mmu.write8(ptr, self.regs.a());
        pc + 2
      }
      ...
      0b1111_0000 => {
        let ptr = (0xFF00 | self.read_arg8() as u16) as usize;
        self.regs.set_a(self.mmu.read8(ptr));
        pc + 2
      }
  ...
  fn exec_cb(&mut self, opcode: u8, pc: u16) -> u16 {
    match opcode {
      ...
      0b0000_0110 => {
        ...
        let v = self.alu_rlc(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0000_0111 => {
        let v = self.alu_rlc(self.regs.a());
      ...
      0b0000_1110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_rrc(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0000_1111 => {
        let v = self.alu_rrc(self.regs.a());
      ...
      0b0001_0110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_rl(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0001_0111 => {
        let v = self.alu_rl(self.regs.a());
      ...
      0b0001_1110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_rr(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0001_1111 => {
        let v = self.alu_rr(self.regs.a());
      ...
      0b0010_0110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_sla(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0010_0111 => {
      ...
      0b0010_1110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_sra(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0010_1111 => {
      ...
      0b0011_0110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_swap(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0011_0111 => {
      ...
      0b0011_1110 => {
        let ptr = self.regs.hl() as usize;
        let v = self.alu_srl(self.mmu.read8(ptr));
        self.mmu.write8(ptr, v);
      }
      0b0011_1111 => {
      ...
      _ if opcode_match(opcode, 0b1100_0111, 0b0100_0110) => {
        ...
        let v = self.mmu.read8(self.regs.hl() as usize);
      ...
      _ if opcode_match(opcode, 0b1100_0111, 0b1000_0110) => {
        ...
        self.mmu.write8(self.regs.hl() as usize, v & !(1 << n));
      }
  ...
  fn push(&mut self, value: u16) {
    ...
    self.mmu.write16(self.regs.sp() as usize, value);
  }

  fn pop(&mut self) -> u16 {
    let v = self.mmu.read16(self.regs.sp() as usize);
    self.regs.set_sp(self.regs.sp() + 2);
  ...
  fn read_arg8(&self) -> u8 {
    ...
    self.mmu.read8((pc + 1) as usize)
  }

  fn read_arg16(&self) -> u16 {
    let pc = self.regs.read16(PC);

    self.mmu.read16((pc + 1) as usize)
  }
...
mod tests {
  ...
  macro_rules! exec {
    ($cpu:expr, $instr:expr) => {{
      ...
      $cpu.mmu._load8(pc, $instr);
      $cpu.exec();
    }};

    ($cpu:expr, $instr:expr, arg8 => $arg8:expr) => {{
      let pc = $cpu.regs.pc() as usize;
      $cpu.mmu._load8(pc, $instr);
      $cpu.mmu._load8(pc + 1, $arg8);
      $cpu.exec();
    }};

    ($cpu:expr, $instr:expr, arg16 => $arg16:expr) => {{
      let pc = $cpu.regs.pc() as usize;
      $cpu.mmu._load8(pc, $instr);
      $cpu.mmu._load16(pc + 1, $arg16);
      $cpu.exec();
    }};
  }
  ...
  fn opcode_ld_ptr16_sp() {
    ...
    assert_eq!(cpu.mmu.read16(0xff90), 2047);
  }
  ...
  fn opcode_ld_r16_n16() {
    ...
    exec!(cpu, 0x21, arg16 => 2047);
    exec!(cpu, 0x31, arg16 => 4095);
  ...
  fn opcode_ld_r16_a() {
    ...
    exec!(cpu, 0x02);
    assert_eq!(cpu.mmu.read8(0xff90), 127);

    // LD DE, A
    cpu.regs.write8(A, 63);
    cpu.regs.write16(DE, 0xff90);
    exec!(cpu, 0x12);
    assert_eq!(cpu.mmu.read8(0xff90), 63);
  }
  ...
  fn opcode_inc_r8() {
    ...
    cpu.mmu.write8(0xff90, 7);
    exec!(cpu, 0x34);
    assert_eq!(cpu.mmu.read8(0xff90), 8);
  ...
  fn opcode_dec_r8() {
    ...
    cpu.mmu.write8(0xff90, 7);
    exec!(cpu, 0x35);
    assert_eq!(cpu.mmu.read8(0xff90), 6);
  ...
  fn opcode_ld_r8_n8() {
    ...
    // LD C, 2
    exec!(cpu, 0b00_001_110, arg8 => 2);
    assert_eq!(cpu.regs.read8(C), 2);

    // LD D, 3
    exec!(cpu, 0b00_010_110, arg8 => 3);
    assert_eq!(cpu.regs.read8(D), 3);

    // LD E, 4
    exec!(cpu, 0b00_011_110, arg8 => 4);
    assert_eq!(cpu.regs.read8(E), 4);

    // LD H, 5
    exec!(cpu, 0b00_100_110, arg8 => 5);
    assert_eq!(cpu.regs.read8(H), 5);

    // LD L, 6
    exec!(cpu, 0b00_101_110, arg8 => 6);
    assert_eq!(cpu.regs.read8(L), 6);

    // LD (HL), 7
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b00_110_110, arg8 => 7);
    assert_eq!(cpu.mmu.read16(0xff90), 7);

    // LD A, 8
    exec!(cpu, 0b00_111_110, arg8 => 8);
  ...
  fn opcode_rdca() {
    ...
    exec!(cpu, 0b0000_0111);
    assert_eq!(cpu.regs.a(), 0b0000_0100);

    // RRCA
    cpu.regs.set_a(0b0000_0010);
    exec!(cpu, 0b0000_1111);
  ...
  fn opcode_jr_f_n() {
    ...
    exec!(cpu, 0b0010_0000, arg8 => 0b0000_1000);
    ...
    cpu.regs.set_pc(0);
    cpu.regs.set_flag(ZF, true);
    exec!(cpu, 0b0010_1000, arg8 => 0b0000_1000);
    assert_eq!(cpu.regs.pc(), 8);
    ...
    cpu.regs.set_pc(0);
    cpu.regs.set_flag(CF, false);
    exec!(cpu, 0b0011_0000, arg8 => 0b0000_1000);
    assert_eq!(cpu.regs.pc(), 8);
    ...
    cpu.regs.set_pc(0);
    cpu.regs.set_flag(CF, false);
    exec!(cpu, 0b0011_1000, arg8 => 0b0000_1000);
    assert_eq!(cpu.regs.pc(), 2);
  ...
  fn opcode_ldi_hl_a() {
    ...
    cpu.regs.set_hl(0xff90);
    cpu.regs.set_a(2);
    exec!(cpu, 0b0010_0010);
    assert_eq!(cpu.mmu.read8(0xff90), 2);
  ...
  fn opcode_daa_flags() {
    ...
    cpu.regs.set_flag(CF, true);
    cpu.regs.set_a(0xA0);
    exec!(cpu, 0b0010_0111, arg8 => 0b0010_0111);
    assert_eq!(cpu.regs.get_flag(ZF), true);
  ...
  fn opcode_ld_b_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0100_0001);
    assert_eq!(cpu.regs.b(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0100_0010);
    assert_eq!(cpu.regs.b(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0100_0011);
    assert_eq!(cpu.regs.b(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0100_0100);
    assert_eq!(cpu.regs.b(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0100_0101);
    assert_eq!(cpu.regs.b(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0100_0110);
    assert_eq!(cpu.regs.b(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0100_0111);
  ...
  fn opcode_ld_c_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0100_1001);
    assert_eq!(cpu.regs.c(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0100_1010);
    assert_eq!(cpu.regs.c(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0100_1011);
    assert_eq!(cpu.regs.c(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0100_1100);
    assert_eq!(cpu.regs.c(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0100_1101);
    assert_eq!(cpu.regs.c(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0100_1110);
    assert_eq!(cpu.regs.c(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0100_1111);
  ...
  fn opcode_ld_d_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0101_0001);
    assert_eq!(cpu.regs.d(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0101_0010);
    assert_eq!(cpu.regs.d(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0101_0011);
    assert_eq!(cpu.regs.d(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0101_0100);
    assert_eq!(cpu.regs.d(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0101_0101);
    assert_eq!(cpu.regs.d(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0101_0110);
    assert_eq!(cpu.regs.d(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0101_0111);
  ...
  fn opcode_ld_e_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0101_1001);
    assert_eq!(cpu.regs.e(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0101_1010);
    assert_eq!(cpu.regs.e(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0101_1011);
    assert_eq!(cpu.regs.e(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0101_1100);
    assert_eq!(cpu.regs.e(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0101_1101);
    assert_eq!(cpu.regs.e(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0101_1110);
    assert_eq!(cpu.regs.e(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0101_1111);
  ...
  fn opcode_ld_h_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0110_0001);
    assert_eq!(cpu.regs.h(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0110_0010);
    assert_eq!(cpu.regs.h(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0110_0011);
    assert_eq!(cpu.regs.h(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0110_0100);
    assert_eq!(cpu.regs.h(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0110_0101);
    assert_eq!(cpu.regs.h(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0110_0110);
    assert_eq!(cpu.regs.h(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0110_0111);
  ...
  fn opcode_ld_l_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0110_1001);
    assert_eq!(cpu.regs.l(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0110_1010);
    assert_eq!(cpu.regs.l(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0110_1011);
    assert_eq!(cpu.regs.l(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0110_1100);
    assert_eq!(cpu.regs.l(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0110_1101);
    assert_eq!(cpu.regs.l(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0110_1110);
    assert_eq!(cpu.regs.l(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0110_1111);
  ...
  fn opcode_ld_hl_r8() {
    ...
    cpu.regs.set_b(1);
    exec!(cpu, 0b0111_0000);
    assert_eq!(cpu.mmu.read8(0xff90), 1);

    cpu.regs.set_c(2);
    exec!(cpu, 0b0111_0001);
    assert_eq!(cpu.mmu.read8(0xff90), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0111_0010);
    assert_eq!(cpu.mmu.read8(0xff90), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0111_0011);
    assert_eq!(cpu.mmu.read8(0xff90), 4);

    cpu.regs.set_h(0xff);
    exec!(cpu, 0b0111_0100);
    assert_eq!(cpu.mmu.read8(cpu.regs.hl() as usize), 0xff);

    cpu.regs.set_l(0x90);
    exec!(cpu, 0b0111_0101);
    assert_eq!(cpu.mmu.read8(cpu.regs.hl() as usize), 0x90);

    cpu.regs.set_a(7);
    exec!(cpu, 0b0111_0111);
    assert_eq!(cpu.mmu.read8(cpu.regs.hl() as usize), 7);
  }
  ...
  fn opcode_ld_a_r8() {
    ...
    cpu.regs.set_c(2);
    exec!(cpu, 0b0111_1001);
    assert_eq!(cpu.regs.a(), 2);

    cpu.regs.set_d(3);
    exec!(cpu, 0b0111_1010);
    assert_eq!(cpu.regs.a(), 3);

    cpu.regs.set_e(4);
    exec!(cpu, 0b0111_1011);
    assert_eq!(cpu.regs.a(), 4);

    cpu.regs.set_h(5);
    exec!(cpu, 0b0111_1100);
    assert_eq!(cpu.regs.a(), 5);

    cpu.regs.set_l(6);
    exec!(cpu, 0b0111_1101);
    assert_eq!(cpu.regs.a(), 6);

    cpu.mmu.write8(0xff90, 7);
    cpu.regs.set_hl(0xff90);
    exec!(cpu, 0b0111_1110);
    assert_eq!(cpu.regs.a(), 7);

    cpu.regs.set_a(8);
    exec!(cpu, 0b0111_1111);
  ...
  fn opcode_add() {
    ...
    cpu.regs.set_a(1);
    cpu.regs.set_b(2);
    exec!(cpu, 0b1000_0000);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, C
    cpu.regs.set_a(1);
    cpu.regs.set_c(2);
    exec!(cpu, 0b1000_0001);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, D
    cpu.regs.set_a(1);
    cpu.regs.set_d(2);
    exec!(cpu, 0b1000_0010);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, E
    cpu.regs.set_a(1);
    cpu.regs.set_e(2);
    exec!(cpu, 0b1000_0011);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, H
    cpu.regs.set_a(1);
    cpu.regs.set_h(2);
    exec!(cpu, 0b1000_0100);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, L
    cpu.regs.set_a(1);
    cpu.regs.set_l(2);
    exec!(cpu, 0b1000_0101);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, (HL)
    cpu.regs.set_a(1);
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 2);
    exec!(cpu, 0b1000_0110);
    assert_eq!(cpu.regs.a(), 3);

    // ADD A, A
    cpu.regs.set_a(1);
    println!("{}", cpu.regs.pc());
    exec!(cpu, 0b1000_0111);
    assert_eq!(cpu.regs.a(), 2);

    // ADD A, N
    cpu.regs.set_a(1);
    exec!(cpu, 0b1100_0110, arg8 => 2);
    assert_eq!(cpu.regs.a(), 3);
  }
  ...
  fn opcode_adc() {
    ...
    cpu.regs.set_a(1);
    cpu.regs.set_b(2);
    exec!(cpu, 0b1000_1000);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, C
    cpu.regs.set_a(1);
    cpu.regs.set_c(2);
    exec!(cpu, 0b1000_1001);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, D
    cpu.regs.set_a(1);
    cpu.regs.set_d(2);
    exec!(cpu, 0b1000_1010);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, E
    cpu.regs.set_a(1);
    cpu.regs.set_e(2);
    exec!(cpu, 0b1000_1011);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, H
    cpu.regs.set_a(1);
    cpu.regs.set_h(2);
    exec!(cpu, 0b1000_1100);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, L
    cpu.regs.set_a(1);
    cpu.regs.set_l(2);
    exec!(cpu, 0b1000_1101);
    assert_eq!(cpu.regs.a(), 3);
    ...
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 2);
    exec!(cpu, 0b1000_1110);
    assert_eq!(cpu.regs.a(), 3);

    // ADC A, A
    cpu.regs.set_a(1);
    exec!(cpu, 0b1000_1111);
    assert_eq!(cpu.regs.a(), 2);

    // ADD A, N
    cpu.regs.set_a(1);
    exec!(cpu, 0b1100_1110, arg8 => 2);
    assert_eq!(cpu.regs.a(), 3);
  }
  ...
  fn opcode_sub() {
    ...
    cpu.regs.set_a(5);
    cpu.regs.set_b(2);
    exec!(cpu, 0b1001_0000);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, C
    cpu.regs.set_a(5);
    cpu.regs.set_c(2);
    exec!(cpu, 0b1001_0001);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, D
    cpu.regs.set_a(5);
    cpu.regs.set_d(2);
    exec!(cpu, 0b1001_0010);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, E
    cpu.regs.set_a(5);
    cpu.regs.set_e(2);
    exec!(cpu, 0b1001_0011);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, H
    cpu.regs.set_a(5);
    cpu.regs.set_h(2);
    exec!(cpu, 0b1001_0100);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, L
    cpu.regs.set_a(5);
    cpu.regs.set_l(2);
    exec!(cpu, 0b1001_0101);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, (HL)
    cpu.regs.set_a(5);
    cpu.regs.set_hl(1024);
    cpu.mmu._load8(1024, 2);
    exec!(cpu, 0b1001_0110);
    assert_eq!(cpu.regs.a(), 3);

    // SUB A, A
    cpu.regs.set_a(5);
    exec!(cpu, 0b1001_0111);
    assert_eq!(cpu.regs.a(), 0);

    // SUB A, N
    cpu.regs.set_a(5);
    exec!(cpu, 0b1101_0110, arg8 => 2);
    assert_eq!(cpu.regs.a(), 3);
  }
  ...
  fn opcode_sbc() {
    ...
    cpu.regs.set_a(5);
    cpu.regs.set_b(2);
    exec!(cpu, 0b1001_1000);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, C
    cpu.regs.set_a(5);
    cpu.regs.set_c(2);
    exec!(cpu, 0b1001_1001);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, D
    cpu.regs.set_a(5);
    cpu.regs.set_d(2);
    exec!(cpu, 0b1001_1010);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, E
    cpu.regs.set_a(5);
    cpu.regs.set_e(2);
    exec!(cpu, 0b1001_1011);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, H
    cpu.regs.set_a(5);
    cpu.regs.set_h(2);
    exec!(cpu, 0b1001_1100);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, L
    cpu.regs.set_a(5);
    cpu.regs.set_l(2);
    exec!(cpu, 0b1001_1101);
    assert_eq!(cpu.regs.a(), 3);
    ...
    cpu.regs.set_hl(1024);
    cpu.mmu._load8(1024, 2);
    exec!(cpu, 0b1001_1110);
    assert_eq!(cpu.regs.a(), 3);

    // SBC A, A
    cpu.regs.set_a(5);
    exec!(cpu, 0b1001_1111);
    assert_eq!(cpu.regs.a(), 0);

    // SBC A, N
    cpu.regs.set_a(5);
    exec!(cpu, 0b1101_1110, arg8 => 2);
    assert_eq!(cpu.regs.a(), 3);
  }
  ...
  fn opcode_and() {
    ...
    cpu.regs.set_a(5);
    cpu.regs.set_b(3);
    exec!(cpu, 0b1010_0000);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, C
    cpu.regs.set_a(5);
    cpu.regs.set_c(3);
    exec!(cpu, 0b1010_0001);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, D
    cpu.regs.set_a(5);
    cpu.regs.set_d(3);
    exec!(cpu, 0b1010_0010);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, E
    cpu.regs.set_a(5);
    cpu.regs.set_e(3);
    exec!(cpu, 0b1010_0011);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, H
    cpu.regs.set_a(5);
    cpu.regs.set_h(3);
    exec!(cpu, 0b1010_0100);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, L
    cpu.regs.set_a(5);
    cpu.regs.set_l(3);
    exec!(cpu, 0b1010_0101);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, (HL)
    cpu.regs.set_a(5);
    cpu.regs.set_hl(1024);
    cpu.mmu._load8(1024, 3);
    exec!(cpu, 0b1010_0110);
    assert_eq!(cpu.regs.a(), 1);

    // AND A, A
    cpu.regs.set_a(5);
    exec!(cpu, 0b1010_0111);
    assert_eq!(cpu.regs.a(), 5);

    // AND A, N
    cpu.regs.set_a(5);
    exec!(cpu, 0b1110_0110, arg8 => 9);
    assert_eq!(cpu.regs.a(), 1);
  }
  ...
  fn opcode_xor() {
    ...
    cpu.regs.set_a(5);
    cpu.regs.set_b(3);
    exec!(cpu, 0b1010_1000);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, C
    cpu.regs.set_a(5);
    cpu.regs.set_c(3);
    exec!(cpu, 0b1010_1001);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, D
    cpu.regs.set_a(5);
    cpu.regs.set_d(3);
    exec!(cpu, 0b1010_1010);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, E
    cpu.regs.set_a(5);
    cpu.regs.set_e(3);
    exec!(cpu, 0b1010_1011);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, H
    cpu.regs.set_a(5);
    cpu.regs.set_h(3);
    exec!(cpu, 0b1010_1100);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, L
    cpu.regs.set_a(5);
    cpu.regs.set_l(3);
    exec!(cpu, 0b1010_1101);
    assert_eq!(cpu.regs.a(), 6);
    ...
    cpu.regs.set_hl(1024);
    cpu.mmu._load8(1024, 3);
    exec!(cpu, 0b1010_1110);
    assert_eq!(cpu.regs.a(), 6);

    // XOR A, A
    cpu.regs.set_a(5);
    exec!(cpu, 0b1010_1111);
    assert_eq!(cpu.regs.a(), 0);

    // XOR A, N
    cpu.regs.set_a(5);
    exec!(cpu, 0b1110_1110, arg8 => 3);
    assert_eq!(cpu.regs.a(), 6);
  }
  ...
  fn opcode_or() {
    ...
    cpu.regs.set_a(5);
    cpu.regs.set_b(3);
    exec!(cpu, 0b1011_0000);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, C
    cpu.regs.set_a(5);
    cpu.regs.set_c(3);
    exec!(cpu, 0b1011_0001);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, D
    cpu.regs.set_a(5);
    cpu.regs.set_d(3);
    exec!(cpu, 0b1011_0010);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, E
    cpu.regs.set_a(5);
    cpu.regs.set_e(3);
    exec!(cpu, 0b1011_0011);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, H
    cpu.regs.set_a(5);
    cpu.regs.set_h(3);
    exec!(cpu, 0b1011_0100);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, L
    cpu.regs.set_a(5);
    cpu.regs.set_l(3);
    exec!(cpu, 0b1011_0101);
    assert_eq!(cpu.regs.a(), 7);
    ...
    cpu.mmu._load8(1024, 3);
    cpu.regs.set_hl(1024);
    exec!(cpu, 0b1011_0110);
    assert_eq!(cpu.regs.a(), 7);

    // OR A, A
    cpu.regs.set_a(5);
    exec!(cpu, 0b1011_0111);
    assert_eq!(cpu.regs.a(), 5);

    // OR A, N
    cpu.regs.set_a(5);
    exec!(cpu, 0b1111_0110, arg8 => 3);
    assert_eq!(cpu.regs.a(), 7);
  }
  ...
  fn opcode_pop() {
    ...
    cpu.mmu._load16(1024, 0xAF);
    exec!(cpu, 0b1100_0001);
  ...
  fn opcode_push() {
    ...
    exec!(cpu, 0b1100_0101);
    assert_eq!(cpu.regs.sp(), 0xff8e);
    assert_eq!(cpu.mmu.read16(0xff8e), 0xAF);
  ...
  fn opcode_call_f_n() {
    ...
    cpu.regs.set_flag(ZF, false);
    cpu.regs.set_sp(0xff90);
    exec!(cpu, 0b1100_0100, arg16 => 123);
    assert_eq!(cpu.regs.sp(), 0xff8e);
    assert_eq!(cpu.mmu.read16(0xff8e), 3);
    assert_eq!(cpu.regs.pc(), 123);
    ...
    cpu.regs.set_flag(ZF, false);
    cpu.regs.set_sp(0xff90);
    exec!(cpu, 0b1100_1100, arg16 => 123);
    assert_eq!(cpu.regs.sp(), 0xff90);
    assert_eq!(cpu.regs.pc(), 3);
    ...
    cpu.regs.set_flag(CF, false);
    cpu.regs.set_sp(0xff90);
    exec!(cpu, 0b1101_0100, arg16 => 123);
    assert_eq!(cpu.regs.sp(), 0xff8e);
    assert_eq!(cpu.mmu.read16(0xff8e), 3);
    assert_eq!(cpu.regs.pc(), 123);
    ...
    cpu.regs.set_flag(CF, false);
    cpu.regs.set_sp(0xff90);
    exec!(cpu, 0b1101_1100, arg16 => 123);
    assert_eq!(cpu.regs.sp(), 0xff90);
    assert_eq!(cpu.regs.pc(), 3);
  ...
  fn opcode_call_n() {
    ...
    // CALL C, N when CF is set
    cpu.regs.set_sp(0xff90);
    exec!(cpu, 0b1100_1101, arg16 => 123);
    assert_eq!(cpu.regs.sp(), 0xff8e);
    assert_eq!(cpu.mmu.read16(0xff8e), 3);
    assert_eq!(cpu.regs.pc(), 123);
  }
  ...
  fn opcode_ld_ff00_n_a() {
    ...
    assert_eq!(cpu.mmu.read8(0xFF80), 1);
  }
  ...
  fn opcode_cb_rl() {
    ...
    cpu.regs.set_b(0b0000_0010);
    cpu.mmu._load8(0, 0xCB);
    exec_cb!(cpu, 0b0001_0000);
    assert_eq!(cpu.regs.b(), 0b0000_0100);

    // RL C
    cpu.regs.set_c(0b0000_0010);
    exec_cb!(cpu, 0b0001_0001);
    assert_eq!(cpu.regs.c(), 0b0000_0100);

    // // RL D
    cpu.regs.set_d(0b0000_0010);
    exec_cb!(cpu, 0b0001_0010);
    assert_eq!(cpu.regs.d(), 0b0000_0100);

    // // RL C
    cpu.regs.set_e(0b0000_0010);
    exec_cb!(cpu, 0b0001_0011);
    assert_eq!(cpu.regs.e(), 0b0000_0100);

    // // RL H
    cpu.regs.set_h(0b0000_0010);
    exec_cb!(cpu, 0b0001_0100);
    assert_eq!(cpu.regs.h(), 0b0000_0100);

    // RL L
    cpu.regs.set_l(0b0000_0010);
    exec_cb!(cpu, 0b0001_0101);
    assert_eq!(cpu.regs.l(), 0b0000_0100);

    // RL (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0b0000_0010);
    exec_cb!(cpu, 0b0001_0110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b0000_0100);

    // RL A
    cpu.regs.set_a(0b0000_0010);
    exec_cb!(cpu, 0b0001_0111);
    assert_eq!(cpu.regs.a(), 0b0000_0100);
  }
  ...
  fn opcode_cb_rr() {
    ...
    // RR B
    cpu.regs.set_b(0b0000_1010);
    exec_cb!(cpu, 0b0001_1000);
    assert_eq!(cpu.regs.b(), 0b0000_0101);

    // RR C
    cpu.regs.set_c(0b0000_1010);
    exec_cb!(cpu, 0b0001_1001);
    assert_eq!(cpu.regs.c(), 0b0000_0101);

    // RR D
    cpu.regs.set_d(0b0000_1010);
    exec_cb!(cpu, 0b0001_1010);
    assert_eq!(cpu.regs.d(), 0b0000_0101);

    // RR C
    cpu.regs.set_e(0b0000_1010);
    exec_cb!(cpu, 0b0001_1011);
    assert_eq!(cpu.regs.e(), 0b0000_0101);

    // RR H
    cpu.regs.set_h(0b0000_1010);
    exec_cb!(cpu, 0b0001_1100);
    assert_eq!(cpu.regs.h(), 0b0000_0101);

    // RR L
    cpu.regs.set_l(0b0000_1010);
    exec_cb!(cpu, 0b0001_1101);
    assert_eq!(cpu.regs.l(), 0b0000_0101);

    // RR (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0b0000_1010);
    exec_cb!(cpu, 0b0001_1110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b0000_0101);

    // RR A
    cpu.regs.set_a(0b0000_1010);
    exec_cb!(cpu, 0b0001_1111);
    assert_eq!(cpu.regs.a(), 0b0000_0101);
  }
  ...
  fn opcode_cb_sra_d() {
    ...
    // SRA B
    cpu.regs.set_b(0b0000_0010);
    exec_cb!(cpu, 0b0010_1000);
    assert_eq!(cpu.regs.b(), 0b0000_0001);

    // SRA C
    cpu.regs.set_c(0b0000_0010);
    exec_cb!(cpu, 0b0010_1001);
    assert_eq!(cpu.regs.c(), 0b0000_0001);

    // SRA D
    cpu.regs.set_d(0b0000_0010);
    exec_cb!(cpu, 0b0010_1010);
    assert_eq!(cpu.regs.d(), 0b0000_0001);

    // SRA E
    cpu.regs.set_e(0b0000_0010);
    exec_cb!(cpu, 0b0010_1011);
    assert_eq!(cpu.regs.e(), 0b0000_0001);

    // SRA H
    cpu.regs.set_h(0b0000_0010);
    exec_cb!(cpu, 0b0010_1100);
    assert_eq!(cpu.regs.h(), 0b0000_0001);

    // SRA L
    cpu.regs.set_l(0b0000_0010);
    exec_cb!(cpu, 0b0010_1101);
    assert_eq!(cpu.regs.l(), 0b0000_0001);

    // SRA (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0b0000_0010);
    exec_cb!(cpu, 0b0010_1110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b0000_0001);

    // SRA A
    cpu.regs.set_a(0b0000_0010);
    exec_cb!(cpu, 0b0010_1111);
    assert_eq!(cpu.regs.a(), 0b0000_0001);
  ...
  fn opcode_cb_swap_d() {
    ...
    // SWAP B
    cpu.regs.set_b(0x12);
    exec_cb!(cpu, 0b0011_0000);
    assert_eq!(cpu.regs.b(), 0x21);

    // SWAP C
    cpu.regs.set_c(0x12);
    exec_cb!(cpu, 0b0011_0001);
    assert_eq!(cpu.regs.c(), 0x21);

    // SWAP D
    cpu.regs.set_d(0x12);
    exec_cb!(cpu, 0b0011_0010);
    assert_eq!(cpu.regs.d(), 0x21);

    // SWAP E
    cpu.regs.set_e(0x12);
    exec_cb!(cpu, 0b0011_0011);
    assert_eq!(cpu.regs.e(), 0x21);

    // SWAP H
    cpu.regs.set_h(0x12);
    exec_cb!(cpu, 0b0011_0100);
    assert_eq!(cpu.regs.h(), 0x21);

    // SWAP L
    cpu.regs.set_l(0x12);
    exec_cb!(cpu, 0b0011_0101);
    assert_eq!(cpu.regs.l(), 0x21);

    // SWAP (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0x12);
    exec_cb!(cpu, 0b0011_0110);
    assert_eq!(cpu.mmu.read8(0xff90), 0x21);

    // SWAP A
    cpu.regs.set_a(0x12);
    exec_cb!(cpu, 0b0011_0111);
  ...
  fn opcode_cb_srl_d() {
    ...
    // SRL B
    cpu.regs.set_b(0b0000_0010);
    exec_cb!(cpu, 0b0011_1000);
    assert_eq!(cpu.regs.b(), 0b0000_0001);

    // SRL C
    cpu.regs.set_c(0b0000_0010);
    exec_cb!(cpu, 0b0011_1001);
    assert_eq!(cpu.regs.c(), 0b0000_0001);

    // SRL D
    cpu.regs.set_d(0b0000_0010);
    exec_cb!(cpu, 0b0011_1010);
    assert_eq!(cpu.regs.d(), 0b0000_0001);

    // SRL E
    cpu.regs.set_e(0b0000_0010);
    exec_cb!(cpu, 0b0011_1011);
    assert_eq!(cpu.regs.e(), 0b0000_0001);

    // SRL H
    cpu.regs.set_h(0b0000_0010);
    exec_cb!(cpu, 0b0011_1100);
    assert_eq!(cpu.regs.h(), 0b0000_0001);

    // SRL L
    cpu.regs.set_l(0b0000_0010);
    exec_cb!(cpu, 0b0011_1101);
    assert_eq!(cpu.regs.l(), 0b0000_0001);

    // SRL (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0b0000_0010);
    exec_cb!(cpu, 0b0011_1110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b0000_0001);

    // SRL A
    cpu.regs.set_a(0b0000_0010);
    exec_cb!(cpu, 0b0011_1111);
    assert_eq!(cpu.regs.a(), 0b0000_0001);
  ...
  fn opcode_cb_bit_n_d_flags() {
    ...
    exec_cb!(cpu, 0b0101_0000);
    assert_eq!(cpu.regs.get_flag(ZF), true);

    // BIT N, C resets ZF if bit N is 1
    cpu.regs.set_c(0b0000_0100);
    exec_cb!(cpu, 0b0101_0001);
    assert_eq!(cpu.regs.get_flag(ZF), false);

    // BIT N, (HL) sets ZF if bit N is zero
    cpu.regs.set_hl(123);
    cpu.mmu._load8(123, 0b0000_0000);
    exec_cb!(cpu, 0b0111_1110);
    assert_eq!(cpu.regs.get_flag(ZF), true);

    // BIT N, A resets ZF if bit N is 1
    cpu.regs.set_a(0b1000_0000);
    exec_cb!(cpu, 0b0111_1111);
    assert_eq!(cpu.regs.get_flag(ZF), false);
  }
  ...
  fn opcode_cb_res_n_d() {
    ...
    // RES N, B
    cpu.regs.set_b(0xFF);
    exec_cb!(cpu, 0b1001_0000);
    assert_eq!(cpu.regs.b(), 0b1111_1011);

    // RES N, (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0xFF);
    exec_cb!(cpu, 0b1001_0110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b1111_1011);
  }
  ...
  fn opcode_cb_set_n_d() {
    ...
    // SET N, B
    cpu.regs.set_b(0x00);
    exec_cb!(cpu, 0b1101_0000);
    assert_eq!(cpu.regs.b(), 0b0000_0100);

    // SET N, (HL)
    cpu.regs.set_hl(0xff90);
    cpu.mmu.write8(0xff90, 0x00);
    exec_cb!(cpu, 0b1101_0110);
    assert_eq!(cpu.mmu.read8(0xff90), 0b0000_0100);
  }
}
